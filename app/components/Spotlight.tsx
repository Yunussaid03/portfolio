...
export function Spotlight({ className = "" }: SpotlightProps) {
  return (
    <div className={`pointer-events-none absolute inset-0 overflow-hidden ${className}`}>
      <div className="absolute inset-0 bg-[linear-gradient(120deg,transparent_0%,rgba(255,255,255,0.08)_35%,transparent_100%)]" />
      ...
      {Array.from({ length: 15 }).map((_, i) => (
        <div
          key={`snowflake-${i}`}
          className="snowflake absolute text-xl text-white/50"
          style={{
            left: `${Math.random() * 100}%`,
            top: `-20px`,
            animationDelay: `${Math.random() * 4}s`,
            animation: `snowfall ${10 + Math.random() * 8}s linear infinite`,
          }}
        >
          ❄
        </div>
